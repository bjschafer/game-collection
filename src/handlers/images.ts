...
import {
  CloudflareImagesUploadResult,
  CloudflareImagesGetResult,
  ...
  try {
    ...
    if (checkResponse.ok) {
      ...
      if (imageData.result?.id) {
        ...
        if (testResponse.ok) {
          ...
          console.error(
            `Failed to access games variant: ${testResponse.status} for URL: ${gameVariantUrl}`,
          );
    ...
    if (uploadedUrl) {
      ...
      if (testResponse.ok) {
        ...
        console.error(
          `Failed to access uploaded games variant: ${testResponse.status} for URL: ${uploadedUrl}`,
        );
      }
    }
